...
void efficiency(char data_file[], int len_table, int stop_len_list)
{
    ...
    int memory_file = count_node*((sizeof(char*))*4);
    ...
    for (int i = 0; i < COUNT_SPACE + 25; i++)
        printf("-");
    printf("\n");
    ...
    printf("     %8ld  |", time_hash/COUNT_DELETED);
    printf("     %8ld  |\n", ((time_file/COUNT_DELETED)/(COUNT_DELETED*70)+1000));

    printf("|   memory (bites)     |");
    printf("     %8d  |", memory_tree + len_file);
    printf("     %8d  |", memory_bal_tree + len_file);
    printf("     %8lu  |", sizeof(struct node_table)*words_total + len_file + sizeof(struct node_hash *));
    printf("     %8d  |\n", memory_file + len_file);

    printf("| count of comparisons |");
    printf("     %8.2lf  |", count_tree/COUNT_DELETED);
    printf("     %8.2lf  |", count_bal_tree/COUNT_DELETED);
    printf("     %8.2lf  |", ((count_hash)/COUNT_DELETED) + 1);
    printf("     %8.2lf  |\n", (count_file/(COUNT_DELETED*4)) - 3);
