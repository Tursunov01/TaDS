...
//void findMinDiff(array_t array, array_t end_array, int* diff, int** pos1, int** pos2)
//{
////    int* tmp;
...
//    *pos1 = array;
//    int i = 1;
//    for (array_t pj = array; pj < end_array; pj++, i++)
//    {
//       if (abs(*pj - 17) <= *diff)
//       {
//           *pos2 = *pos1;
...
//           *diff = abs(17 - *pj);
////           printf("diff = %d, Pos1 = %d, pos2 = %d\n",*diff, *pos1, *pos2);
//       }
//    }

////   printf("Pos1 = %d, pos2 = %d", *pos1, *pos2);
//}
//double compare_double(void const *val1, void const *val2)
//{
...
//{
//    int diff, *pos1, *pos2;
//   int arr[] = {17, 16, 21, 0, 18, 17};
////   int n = sizeof(arr)/sizeof(arr[0]);
//    findMinDiff(arr,  arr + 6, &diff, &pos1, &pos2);
//     printf("FINAL Pos1 = %d, pos2 = %d", *pos1, *pos2);
////    double num1 = 1.;
////    double num2 = 1.2;
////   printf("HMM = %lf", compare_double(&num1,&num2));
//   return 0;
//}
#include <stdio.h>
#include <string.h>
...
void info(void); // Функция справка
...
int main(void)
{
    ...
    printf("Введите число (делимое) и нажмите Enter:\n\
        10        20        30      \n\
|--------|---------|---------|E----|\n");
...
|--------|---------|---------|E----|\n");
    ...
    printf("\nВведите второе число (делитель) и нажмите Enter:\n\
        10        20        30      \n\
|--------|---------|---------|E----|\n");
...
|--------|---------|---------|E----|\n\n");
    ...
    printf("Результат: %s / %s = ", str_nums[0], str_nums[1]);
    data[2] = big_division(data[0], data[1]);
//    printf("STEPENI = %d\n", res.exp_num);
    if (abs(data[2].exp_num) > 99999)// || (res.exp_num < -99999) )
        data[2].error_code = TOO_BIG_E_ERROR;

    if (!data[2].error_code)
        pretty_print(data);
    else
    {
        printf("Ошибка. \nВ результате деления получено некорректное число (выходящее за диапазон) или было произведено деление на ноль.\n");
...
//    printf("STEPENI = %d\n", res.exp_num);
    if (abs(res.exp_num) > 99999)// || (res.exp_num < -99999) )
    {
        res.error_code = TOO_BIG_E_ERROR;
        return res;
    }
...
void info(void)
{
    printf("Программа производит деление двух вещественных чисел,"
           "выходящих за разрядную сетку.\n\n");
    printf("Формат ввода чисел:\n");
    printf("1. Допускается ввод как с '.' так и без: .00025, +123001, –123.456\n");
    printf("2. Степень вводится СТРОГО с большой буквы 'E'\n");
    printf("3. Допускается ввод без знака степени 'E': 13.37, -228\n");
    printf("4. Допускается ввод в экспоненциальном формате: 1234567Е–20, 1234567Е20 или -123.4567Е23\n");
    printf("5. Посторонние символы в записи числа будут проигнорированы программой,\n");
    printf("   однако программа не завершится и пользователю будет предложена ещё одна попытка ввода.\n\n");
}
